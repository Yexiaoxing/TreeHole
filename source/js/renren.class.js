 ...
 */ (function (window) {
    ...
    var Renren = {
        ...
        init: function (options) {
            this.options = merge.apply(null, append([{},
                this.options
            ], arguments));
            ...
            if (!Browser.browser) {
                var ua = navigator.userAgent.toLowerCase(),
                    platform = navigator.platform
                        .toLowerCase(),
                    UA = ua
                        .match(/(opera|ie|firefox|chrome|version)[\s\/:]([\w\d\.]+)?.*?(safari|version[\s\/:]([\w\d\.]+)|$)/) || [null, 'unknown', 0],
                ...
                Browser.browser = {
                    ...
                    platform: ua.match(/ip(?:ad|od|hone)/) ? 'ios' : (ua
                        .match(/(?:webos|android)/) || platform.match(/mac|win|linux/) || ['other'])[0]
                }
            }
    ...
    var Flash = {
        ...
        init: function () {
            ...
            Flash.inited = true;
            var html = ('<object type="application/x-shockwave-flash" id="' + Flash.name + '" data="' + Flash.url + '"' + ( !! document.attachEvent ? ' name="' + Flash.name + '" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000"' : '') + ' width="0" height="0"><param name="movie" value="' + Flash.url + '"></param><param name="allowScriptAccess" value="always"></param><param name="flashVars" value="onReady=Renren.onFlashReady"/></object>');
    ...
    var Dom = {
        ...
        insertIframe: function (opts) {
            ...
            if (document.attachEvent) {
                var html = ('<iframe id="' + opts.id + '" name="' + opts.name + '"' + (opts.className ? ' class="' + opts.className + '"' : '') + ' style="border:none;' + (opts.width ? 'width:' + opts.width + 'px;' : '') + (opts.height ? 'height:' + opts.height + 'px;' : '') + '" src="' + opts.url + '" frameborder="0" scrolling="no" allowtransparency="true"' + ' onload="Renren.onIframeReady(\'' + opts.id + '\')"' + '></iframe>');
        ...
        popupWindow: function (opts) {
            var screenX = typeof window.screenX != 'undefined' ? window.screenX : window.screenLeft,
                ...
                width = opts.width,
                height = opts.height,
                left = opts.left != null ? opts.left : parseInt(screenX + ((outerWidth - width) / 2), 10),
                top = opts.top != null ? opts.top : parseInt(screenY + ((outerHeight - height) / 2.5), 10),
        ...
        createUI: function (opts) {
            ...
            if (bws.name == 'ie' && bws.version < 9) {
                forEach(['top_left', 'top_right', 'bottom_left', 'bottom_right'], function (item) {
                    ...
                    style.background = 'url(' + rrUrls.rrstatic + 'connect/img/dialog/pop_dialog_' + item + '.png) no-repeat 0 0';
            ...
            var view = Dom.getViewportInfo(),
                left = opts.left != null ? opts.left + (bws.name == 'ie' ? 10 : 0) : (view.scrollLeft + (view.width - opts.width - 20) / 2),
                top = opts.top != null ? opts.top + (bws.name == 'ie' ? 10 : 0) : (view.scrollTop + (view.height - opts.height - 20) / 2.5);
        ...
        getViewportInfo: function () {
            var d = (document.documentElement && document.compatMode == 'CSS1Compat') ? document.documentElement : document.body;
            return {
     ...
     */ {
        ...
        init: function (options) {
            if (options && options.originPath != undefined)
                XD.origin = location.protocol + '//' + location.host + '/' + options.originPath;
        ...
        resolveRelation: function (relation) {
            var pt, matches, parts = relation.split('.'),
                node = window;
            ...
            init: function () {
                Flash.onReady(function () {
                    XD.transports.flash = Flash.initPostMessage(
                        'Renren.XD.Flash.receiveMessage', XD.origin) ? 'flash' : false;
                });
            },
        ...
        Fragment: {
            ...
            sendMessage: function (message, targetOrigin, targetWin) {
                if (targetOrigin && targetOrigin.indexOf(targetWin.location.protocol + '//' + targetWin.location.host + '/') == 0)
        ...
        sendMessage: function (message, targetOrigin, transport, targetRelWin) {
            if (targetOrigin && transport && transport.length > 0) {
                transport = transport.substr(0, 1).toUpperCase() + transport.substr(1);
                try {
                    if (typeof targetRelWin == 'string') {
                        targetRelWin = XD.resolveRelation(targetRelWin);
                    }
                    XD[transport] && XD[transport].sendMessage(message, targetOrigin,
                        targetRelWin);
                } catch (e) {}
            }
        },
        ...
        dispatchLocationFragment: function (loc) {
            ...
            if (loc.indexOf('#') == -1)
                return;
            var fragment = loc.substr(loc.indexOf('#') + 1),
                magicIndex = fragment
                    .indexOf(XD.Fragment.magic),
                params = fromQueryString(fragment);
            if (magicIndex > 0) {
                document.documentElement.style.display = 'none';
            }
            params.origin && XD.sendMessage(params, params.origin, params.transport,
                params.relation);
        }
    };
    ...
    var XDResponse = {
        ...
        handleXDMessages: function (msg) {
            if (msg.data && msg.data.rqid) {
                ...
                if (req && req.xd) {
                    if (req.xd.transport != 'fragment' && (!msg.origin || msg.origin
     ...
     */ {
        ...
        createUI: function () {
            var opts = this.options,
                method = opts.method,
                url = (method === 'GET' ? opts.url + (opts.url.indexOf('?') < 0 ? '?' : '&') + toQueryString(opts.params) : 'about:blank');
            if (method === 'GET') {
                var bn = Browser.getBrowser().name,
                    maxLgh = (bn == 'ie' ? 2050 : 7600);
     ...
     */ {
         ...
         */
        getRedirectUri: function () {
            var opts = merge(this.xd, 'rqid', this.id),
                trans = this.xd.transport,
                uri;
     ...
     */ {
        ...
        send: function () {
            ...
            if (this.ui && params['redirect_uri'] && params['redirect_uri'].indexOf(rrUrls.callback) == 0) {
    ...
    extend(Request.Popup, {
        ...
        popupMonitor: function () {
            ...
            for (var id in this.popupReqs) {
                if (this.popupReqs.hasOwnProperty(id) && this.popupReqs[id] instanceof this) {
                    try {
    ...
    var ui = function (options, cb) {
        ...
        var url = options.url,
            url = (url.indexOf('http://') == 0 || url
                .indexOf('https://') == 0) ? url : (rrUrls.widget + 'dialog/' + url);
        var p = /^(https?:\/\/[^\/]*\/)([^\/\?#]*\/)*([^\/\?#]*)/.exec(url);
        if (p == null || (rrUrls.widget != p[1] && rrUrls.graph != p[1] && rrUrls.graph_https != p[1])) {
            return;
        }
        var reqOptns = combine({
            ...
            params: {
                app_id: rrUrls.widget == p[1] ? Renren.options.appId : null,
                client_id: (rrUrls.graph == p[1] || rrUrls.graph_https == p[1]) ? Renren.options.appId : null
            },
            onComplete: cb
        });

        return (function (reqOpts) {
            var display = reqOpts.display,
                request;
            if (display.length > 0) {
            ...
            try {
                ...
                if (options.fallback !== false) {
                    ...
                    } else if (e.name == 'UICreateError' && request instanceof Request.Popup) {
    ...
    var XDPC = {
         ...
         */ {
            ...
            addMethod: function (name, fn, orgns) {
                if (!this.methods[name]) {
                    this.methods[name] = XDPC.proxy(name, fn, orgns && typeOf(orgns) != 'array' ? [orgns] : orgns);
                }
            },
        ...
        proxy: function (name, fn, origins) {
            return function (msg) {
                if (origins && origins.length > 0 && !XDPC.checkOrigin(msg.origin, origins))
                    return;
    ...
    XDPC.CanvasClient = new Class({
        ...
        initialize: function () {
            var s = location.search,
                params = (s == '' ? {} : fromQueryString(s.substr(1)));
            this.parent({
