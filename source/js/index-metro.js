$(document).ready(function(){
	...
	resizeBg = function() {
		if ( theWindow.width() < 760 ) {
			$('.class_div_header').stop().animate({'height':'20px','padding-top':'20px'},500);
			...
			$('.class_div_alert').stop().animate({'min-height':'50px','width':theWindow.width()-20+'px'},500);
			...
			$('.class_div_header').stop().animate({'height':'120px','padding-top':'40px'},500);
			$('.class_textarea_status').stop().animate({width:'734px',height:'148px'},500);
			$('.class_button').stop().animate({width:'754px'},500);
			$('.class_div_alert').stop().animate({'min-height':'110px','width':theWindow.width()-20+'px'},500);
...
function showAlert(sMsg,iStyle,iTime){
	...
	$('#id_a_tip').stop(true,true).css('visibility','visible').hide().text(sMsg).fadeIn(400);
	if( iTime&&!closeTimer ) closeTimer = setTimeout(closeAlert,iTime);
	$("#btn_send").css("cursor","pointer");
}
function closeAlert(){
	closeTimer = 0;
	$('#id_a_tip').stop(true,true).fadeOut(1000);
}